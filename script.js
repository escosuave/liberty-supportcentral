...
document.addEventListener('DOMContentLoaded', function () {
  const TARGET_FORM_IDS = new Set(['28683691128983']); // your form id
  ...
  function insertCallout(targetTextarea) {
    if (callout) return;

    callout = document.createElement('div');
    callout.className = 'ltx-callout';
    callout.innerHTML = `
      <div class="ltx-callout-body">
        <strong>Heads up!</strong><br>
        Don’t forget to add your <em>Job Title</em> and <em>Department</em> in this box.
      </div>
      <div class="ltx-callout-arrow" aria-hidden="true"></div>
    `;
    ...
    const position = () => {
      ...
      const gap = 24;
      callout.style.left = rect.left + window.scrollX - cw - gap + 'px';
      callout.style.top  = rect.top + window.scrollY + (rect.height - ch) / 2 + 'px';
    };
  ...
  async function maybeShowCallout() {
    const activeId = getActiveFormId();

    // Remove if wrong form
    if (callout && (!activeId || !TARGET_FORM_IDS.has(activeId))) {
      callout.remove(); callout = null;
    }

    if (!activeId || !TARGET_FORM_IDS.has(activeId)) return;

    // Insert callout
    const desc = await waitFor('#request_description');
    insertCallout(desc);

    // Prefill Subject
    const subjectInput = await waitFor('#request_subject');
    if (subjectInput && subjectInput.value.trim() === '') {
      subjectInput.value = 'Support Central License Request';
    }
  }

  maybeShowCallout();
  const obs = new MutationObserver(() => maybeShowCallout());
  obs.observe(document.body, { childList: true, subtree: true });
